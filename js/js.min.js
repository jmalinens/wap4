...
{$('#percents').html("100");$("#percents").append("%");$('#percents').width(400);var fails=test+"-"+opt_uniqid+"."+getFormat($("input[name='format']:checked").val());var saite_uz_failu=opt_download+': <a href="/files/converted/'+fails+'">'+fails+'</a>';$('#download').html(saite_uz_failu);$('#download').show("slow");clearInterval(refreshIntervalId);}
else
{$('#percents').html(response);$('#percents').width(response*4);$("#percents").append("%");if(response==100||response==99)
{var fails=test+"-"+opt_uniqid+"."+getFormat($("input[name='format']:checked").val());var saite_uz_failu=opt_download+': <a href="/files/converted/'+fails+'">'+fails+'</a>';$('#percents').width(400);$('#download').html(saite_uz_failu);$('#download').show("slow");clearInterval(refreshIntervalId);}}});},2000);$.get("/en/converter/convert/"
+opt_uniqid+"/"

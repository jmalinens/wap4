...
{$('#resize_field').hide();}});var test;$("#conv").live("submit",function(){$('#converter2').hide("slow");$('.media').hide("slow");$('#youtube_uploaded').hide("slow");$('#bar_youtube').hide("slow");$('#bar_convert').show("slow");test=$.ajax({url:"/en/converter/change_body/"+faila_nosaukums,async:false}).responseText;refreshIntervalId=setInterval(function(){$("#percents").load("/en/converter/statuss/"+opt_uniqid,function(response,status,xhr){if(status=="error"){var msg="Извините, произошла ошибка: ";$("#error").html(msg+xhr.status+" "+xhr.statusText);clearInterval(refreshIntervalId);}
...
+"/"+$('#resize').val()
+"/"+$('#width').val()
+"/"+$('#heigth').val(),{input_file:faila_nosaukums,caur_ajax:"true",key:opt_uniqid,format:$("input[name='format']:checked").val(),cut:$('input:checkbox:checked').val(),s_hh:$('#s_hh').val(),s_mm:$('#s_mm').val(),s_ss:$('#s_ss').val(),e_hh:$('#e_hh').val(),e_mm:$('#e_mm').val(),e_ss:$('#e_ss').val(),descr:$('#apraksts').val(),resize:$('#resize').val(),width:$('#width').val(),heigth:$('#heigth').val()},function(data){clearInterval(refreshIntervalId);$('#percents').html("100%");$('#percents').width(400);$('#bar_convert').hide();var fails=data+"-"+opt_uniqid+"."+getFormat($("input[name='format']:checked").val());var saite_uz_failu='Файл успешно преобразован. Ссылка для скачивания: <a href="/files/converted/'+fails+'">'+fails+'</a>';$('#download').html(saite_uz_failu);$('#download').show("slow");});return false;});});var _gaq=_gaq||[];_gaq.push(['_setAccount','UA-4906479-3']);_gaq.push(['_trackPageview']);(function(){var ga=document.createElement('script');ga.type='text/javascript';ga.async=true;ga.src=('https:'==document.location.protocol?'https://ssl':'http://www')+'.google-analytics.com/ga.js';var s=document.getElementsByTagName('script')[0];s.parentNode.insertBefore(ga,s);})();
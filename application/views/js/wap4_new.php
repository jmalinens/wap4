...
console.log(faila_nosaukums);
        ...
        messages: {
            typeError: "{file}- <?=lang("fail.extension")?> ({extensions}).",
            sizeError: "{file}- <?=lang("fail.size")?>, max: {sizeLimit}.",
...
$("#youtube").live("submit", function(){
    ...
    if($('#youtube_link').val().indexOf("youtube.com") == -1 && $('#youtube_link').val().indexOf("vimeo.com") == -1) {
        alert("<?=lang("link.incorrect");?>: http://www.youtube.com, http://m.youtube.com, http://vimeo.com");
        return false;
    }
    var website_type;
    ...
    $('#demo').hide("slow");
         ...
         $('#youtube_uploaded').html("<?=lang("upload.wait")?>");
         $('#bar_youtube').show();
